...
# ════════════════════════════════════════════════════════════
    ...
    if to_visit_df.empty:
        ...
        # Search
        search_query = st.text_input("🔍 Search places", placeholder="Type a place name…",
        ...
        if search_query:
            filtered_df = filtered_df[filtered_df['Place Name'].str.contains(search_query, case=False, na=False)]
...
# ════════════════════════════════════════════════════════════
    ...
    if visited_df.empty:
        ...
        # Search
        search_query_v = st.text_input("🔍 Search places", placeholder="Type a place name…",
        ...
        # Apply filters
        v_filtered = visited_df.copy()
        if search_query_v:
            v_filtered = v_filtered[v_filtered['Place Name'].str.contains(search_query_v, case=False, na=False)]
        if v_cat_filter != 'All':
        ...
        if v_filtered.empty:
            ...
            for idx, row in v_filtered.iterrows():
                name = row.get('Place Name', 'Unknown')
                cat  = row.get('Category', '')
                with st.expander(f"✅  {name}  —  {cat}"):
                    d1, d2 = st.columns([2, 1])
